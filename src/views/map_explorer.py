...
from src.state import set_selected_airport
...
def render_map_explorer(df) -> None:
    ...
    header_cols = st.columns([1.55, 3.7, 1.15], vertical_alignment="center")
    with header_cols[0]:
        render_brand()
    with header_cols[1]:
        active_layer = render_layer_selector(st.session_state["active_layer"])
    with header_cols[2]:
        st.markdown("<div class='ad-pill'>Actualizado · dataset mensual</div>", unsafe_allow_html=True)

    active_layer = st.session_state["active_layer"]
    apply_active_layer_styles(active_layer)
    render_layer_context(active_layer)

    filter_col, map_col, detail_col = st.columns([1.05, 4.35, 1.25], gap="large")

    with filter_col:
        filters = render_filters(df, active_layer)

    submetric = filters["climate_metric"] if active_layer == "clima" else filters["connectivity_metric"]
    ...
    with detail_col:
        ...
        if selected_row:
            ...
            render_layer_summary(summary, metric_label, unit, cfg["icon"], cfg["color"], agg_rule)
            render_empty_selection()
    ...
    with map_col:
        ...
        fig = build_trend_chart(series, active_layer, metric_label, unit, selected_label)
